...
// Query principal para cargar listas base.
const QUERY_ALL = `
  query {
...
// Query para cargar reportes.
const QUERY_REPORTES = `
  query {
    reporteClientes { cedula nombres apellidos telefono total_mascotas }
    reporteMedicamentos { id nombre dosis total_mascotas }
  }
`;

// Mutaciones de clientes.
const MUTATION_CREATE_CLIENTE = `
  mutation ($input: ClienteInput!) {
    createCliente(input: $input) { cedula }
  }
`;

const MUTATION_UPDATE_CLIENTE = `
  mutation ($cedula: String!, $input: ClienteInput!) {
    updateCliente(cedula: $cedula, input: $input) { cedula }
  }
`;

const MUTATION_DELETE_CLIENTE = `
  mutation ($cedula: String!) {
    deleteCliente(cedula: $cedula)
  }
`;

// Mutaciones de medicamentos.
const MUTATION_CREATE_MEDICAMENTO = `
  mutation ($input: MedicamentoInput!) {
    createMedicamento(input: $input) { id }
  }
`;

const MUTATION_UPDATE_MEDICAMENTO = `
  mutation ($id: ID!, $input: MedicamentoInput!) {
    updateMedicamento(id: $id, input: $input) { id }
  }
`;

const MUTATION_DELETE_MEDICAMENTO = `
  mutation ($id: ID!) {
    deleteMedicamento(id: $id)
  }
`;

// Mutaciones de mascotas.
const MUTATION_CREATE_MASCOTA = `
  mutation ($input: MascotaInput!) {
    createMascota(input: $input) { id }
  }
`;

const MUTATION_UPDATE_MASCOTA = `
  mutation ($id: ID!, $input: MascotaInput!) {
    updateMascota(id: $id, input: $input) { id }
  }
`;

const MUTATION_DELETE_MASCOTA = `
  mutation ($id: ID!) {
    deleteMascota(id: $id)
  }
`;
...
export default function App() {
  ...
  async function loadAll() {
    try {
      setError("");
      const data = await graphqlRequest(QUERY_ALL);
  ...
  async function loadReportes() {
    try {
      setError("");
      const data = await graphqlRequest(QUERY_REPORTES);
      setReporteClientes(data.reporteClientes);
      setReporteMedicamentos(data.reporteMedicamentos);
    } catch (e) {
      setError(e.message);
      pushAlert("error", e.message);
    }
  ...
  async function submitCliente(e) {
    ...
    try {
      setError("");
      if (editCliente) {
        await graphqlRequest(MUTATION_UPDATE_CLIENTE, { cedula: clienteForm.cedula, input: clienteForm });
        pushAlert("success", "Cliente actualizado");
      } else {
        await graphqlRequest(MUTATION_CREATE_CLIENTE, { input: clienteForm });
  ...
  async function submitMedicamento(e) {
    ...
    try {
      setError("");
      if (editMedicamento) {
        await graphqlRequest(MUTATION_UPDATE_MEDICAMENTO, { id: editMedicamento, input: medicamentoForm });
        pushAlert("success", "Medicamento actualizado");
      } else {
        await graphqlRequest(MUTATION_CREATE_MEDICAMENTO, { input: medicamentoForm });
  ...
  async function submitMascota(e) {
    ...
    try {
      ...
      if (editMascota) {
        await graphqlRequest(MUTATION_UPDATE_MASCOTA, { id: editMascota, input: payload });
        pushAlert("success", "Mascota actualizada");
      } else {
        await graphqlRequest(MUTATION_CREATE_MASCOTA, { input: payload });
  ...
  async function remove(path) {
    try {
      ...
      if (path.startsWith("/clientes/")) {
        const cedula = path.replace("/clientes/", "");
        await graphqlRequest(MUTATION_DELETE_CLIENTE, { cedula });
      } else if (path.startsWith("/medicamentos/")) {
        const id = path.replace("/medicamentos/", "");
        await graphqlRequest(MUTATION_DELETE_MEDICAMENTO, { id: Number(id) });
      } else if (path.startsWith("/mascotas/")) {
        const id = path.replace("/mascotas/", "");
        await graphqlRequest(MUTATION_DELETE_MASCOTA, { id: Number(id) });
      }
